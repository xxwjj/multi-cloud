...
type DBAdapter interface {
	//Policy
	CreatePolicy(ctx *c.Context, pol *model.Policy) error
	DeletePolicy(ctx *c.Context, id string) error
	UpdatePolicy(ctx *c.Context, pol *model.Policy) error
	ListPolicy(ctx *c.Context) ([]model.Policy, error)
	GetPolicy(ctx *c.Context, id string) (*model.Policy, error)
	//Plan
	CreatePlan(ctx *c.Context, conn *model.Plan) error
	DeletePlan(ctx *c.Context, name string) error
	UpdatePlan(ctx *c.Context, conn *model.Plan) error
	...
	//Job
	CreateJob(ctx *c.Context, job *model.Job) error
